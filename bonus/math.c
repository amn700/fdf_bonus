...
void render_map(t_fdf *data)
{
    ...
    while (dims.y < data->height)
	{
		...
		{
            double x = (int)dims.x - (int)(data->map[dims.y].max_x /2);
            double y = (int)dims.y - (int)(data->height /2);
            double z = data->map[dims.y].line[dims.x].z;
            ...
            data->map[dims.y].line[dims.x].final.x = (x * (data->scale_factor/2)) + data->offset_x;
            data->map[dims.y].line[dims.x].final.y = (y * (data->scale_factor/2)) + data->offset_y;
            dims.x++;
        }
        dims.y++;
    }
