...
void draw_diagnol_line(t_fdf data)
{
	t_screen one;
	t_screen two;
	uint32_t y = 0;
...
void draw_diagnol_line_2(t_fdf data)
{
	t_screen one;
	t_screen two;
	uint32_t y = 0;
	uint32_t x = 0;
	while (y + 1 < data.height) // Ensure y+1 is within bounds
	{
		x = 1; // Start from 1 to avoid negative index (x-1)
		while (x < data.map[y].max_x)
		{
			one = data.map[y].line[x].final;
			if (x - 1 >= 0 && x - 1 < data.map[y + 1].max_x)
			{
				two = data.map[y + 1].line[x - 1].final;
				draw_line(one, two, data.img, (t_Color_duo){data.map[y].line[x].color, data.map[y + 1].line[x - 1].color});
			}
...
{
	draw_diagnol_line(data);
	draw_diagnol_line_2(data);
}
